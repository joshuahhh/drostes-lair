...
async function main() {
  ...
  const renderSierpinski = (
    ...
    if (value !== null) {
      ...
      // const aSize =
      //   a === null ? { w: 0, h: 0 } : renderSierpinski(aLyr, a, lensObj?.a);
      // const bLyr = lyr.spawnLater();
      // const bSize =
      //   b === null ? { w: 0, h: 0 } : renderSierpinski(bLyr, b, lensObj?.b);
      // const cLyr = lyr.spawnLater();
      // const cSize =
      //   c === null ? { w: 0, h: 0 } : renderSierpinski(cLyr, c, lensObj?.c);
      const aLyr = lyr.spawnLater();
      const aSize = renderSierpinski(aLyr, a, lensObj?.a);
      const bLyr = lyr.spawnLater();
      const bSize = renderSierpinski(bLyr, b, lensObj?.b);
      const cLyr = lyr.spawnLater();
      const cSize = renderSierpinski(cLyr, c, lensObj?.c);
  ...
  const renderSceneValue = (
    ...
    if (typeof value === "object" && value !== null && "dominoes" in value) {
      ...
      renderSierpinski(subLyr, value as any, lensObj, true);
      lyr.do(() => {
